...
def sofa_aisi(logdir, cfg, df_cpu, df_gpu):
    global iteration_table
###
    filein_memory = logdir + 'pcm_memory.csv'
    memory_fields = ['timestamp', 'skt', 'iMC_Read', 'iMC_Write', 'partial_write', 'iMC_Total', 'sysRead', 'sysWrite', 'sysTotal']
    df_memory = pd.read_csv(filein_memory, names=memory_fields, sep=';')
...
###
        mem_intval_list = []
        for i in range(1,len(times)):
            mcond1 = (df_memory['timestamp'] >= times[i-1])
            mcond2 = (df_memory['timestamp'] <  times[i])
            mem_interval = df_memory[ mcond1 & mcond2 ]
            if len(mem_interval) > 0:
                mem_intval_list.append((mem_interval['iMC_Read'].mean(), mem_interval['iMC_Write'].mean()))
        mem_intval_summary = pd.DataFrame( mem_intval_list, columns=['iMC_Read','iMC_Write'] )
...
###
            mean_mem_rd = mem_intval_summary['iMC_Read'].mean()
            mean_mem_wt = mem_intval_summary['iMC_Write'].mean()
...
###
            print("Averaged memory read bandwidth (MB/s): ", mean_mem_rd)
            print("Averaged memory write bandwidth (MB/s): ", mean_mem_wt)
